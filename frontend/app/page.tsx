    ...
    <ProsjekterShell>
      <div className="flex h-full overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto p-8">
          ...
          {/* KPI row */}
          <motion.div
            initial="hidden" animate="show" variants={kpiVariants}
          ...
          >
            {kpis.map(({ icon, label, value, suffix, color, bg }) => (
              <motion.div
                key={label}
                variants={kpiItem}
                className={`rounded-xl border border-slate-200 dark:border-gray-700 ${bg} p-4`}
              >
          ...
          >
            ...
            {cities.length > 2 && (
              ...
              >
                {cities.map((c) => (
                  <option key={c} value={c}>{c === 'alle' ? 'Alle byer' : c}</option>
                ))}
              </select>
            )}
          </motion.div>
                        ...
                        <div className="flex items-center gap-0">
                          {STEPS.map((s, i) => (
                            <div key={s} className="flex items-center flex-1 last:flex-none">
                              <div className={`flex-none w-2 h-2 rounded-full transition-all ${
                                i <= stepIdx ? STATUS_DOT[p.status] : 'bg-slate-200 dark:bg-gray-700'
                              }`} />
                              {i < STEPS.length - 1 && (
                                <div className={`flex-1 h-px mx-0.5 ${
                                  i < stepIdx ? 'bg-current opacity-40' : 'bg-slate-200 dark:bg-gray-700'
                                }`} />
                              )}
                            </div>
                          ))}
