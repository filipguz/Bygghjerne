...
function AssetHealthPanel({
  ...
  return (
    <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
      <div className="bg-white dark:bg-gray-900 rounded-2xl w-full max-w-md shadow-xl flex flex-col gap-4 p-6 max-h-[85vh] overflow-y-auto">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="font-semibold text-slate-900 dark:text-gray-100">{asset.name}</h2>
            <p className="text-xs text-slate-400 dark:text-gray-500 mt-0.5">Tilstandsanalyse</p>
          </div>
        ...
        {loading ? (
          ...
          <>
            {analysis && (
              <div className="grid grid-cols-3 gap-3">
                <div className="bg-slate-50 dark:bg-gray-800 rounded-xl p-3 text-center">
                  <p className="text-xs text-slate-400 dark:text-gray-500 mb-1">Siste score</p>
                  <p className={`text-xl font-bold ${analysis.latest_score != null && analysis.latest_score <= 2 ? "text-red-600" : analysis.latest_score === 3 ? "text-yellow-600" : "text-green-600"}`}>
                    {analysis.latest_score ?? "—"}<span className="text-sm font-normal text-slate-400 dark:text-gray-500">/5</span>
                  </p>
                </div>
                <div className="bg-slate-50 dark:bg-gray-800 rounded-xl p-3 text-center">
                  <p className="text-xs text-slate-400 dark:text-gray-500 mb-1">Snitt</p>
                  <p className="text-xl font-bold text-slate-700 dark:text-gray-200">
                    {analysis.average_score ?? "—"}<span className="text-sm font-normal text-slate-400 dark:text-gray-500">/5</span>
                  </p>
                </div>
                <div className="bg-slate-50 dark:bg-gray-800 rounded-xl p-3 text-center">
                  <p className="text-xs text-slate-400 dark:text-gray-500 mb-1">Rapporter</p>
                  <p className="text-xl font-bold text-slate-700 dark:text-gray-200">{analysis.report_count}</p>
                </div>
              </div>
            )}
            ...
            {trend.length >= 2 && (
              <div>
                <p className="text-xs font-medium text-slate-500 dark:text-gray-400 mb-2">Tilstandstrend</p>
                <ConditionSparkline trend={trend} />
                <div className="flex justify-between mt-1">
                  <p className="text-xs text-slate-400 dark:text-gray-500">{trend[trend.length - 1]?.report_date}</p>
                  <p className="text-xs text-slate-400 dark:text-gray-500">{trend[0]?.report_date}</p>
                </div>
              </div>
            )}
            ...
            {analysis && analysis.patterns.length > 0 && (
              <div className="flex flex-col gap-2">
                <p className="text-xs font-medium text-slate-500 dark:text-gray-400">Mønstre oppdaget</p>
            ...
            {trend.length > 0 && (
              <div>
                <p className="text-xs font-medium text-slate-500 dark:text-gray-400 mb-2">Siste {Math.min(trend.length, 4)} rapporter</p>
                <div className="flex flex-col gap-1.5">
                  {trend.slice(0, 4).map((r, i) => (
                    <div key={i} className="flex items-center justify-between text-xs text-slate-600 dark:text-gray-400 py-1.5 border-b border-slate-100 dark:border-gray-800 last:border-0">
                      <span className="text-slate-400 dark:text-gray-500">{new Date(r.report_date).toLocaleDateString("nb-NO")}</span>
                      <span className="text-slate-500 dark:text-gray-400">{REPORT_TYPE_LABELS[r.report_type] ?? r.report_type}</span>
            ...
            {analysis?.report_count === 0 && (
              <p className="text-sm text-slate-400 dark:text-gray-500 text-center py-4">Ingen inspeksjonsrapporter ennå</p>
            )}
...
function CreateReportModal({
  ...
  return (
    <div className="fixed inset-0 bg-black/40 z-50 flex items-end sm:items-center justify-center p-4">
      <div className="bg-white dark:bg-gray-900 rounded-2xl w-full max-w-lg shadow-xl p-6 flex flex-col gap-4 max-h-[92vh] overflow-y-auto">
        <div className="flex items-center justify-between">
          <h2 className="font-semibold text-slate-900 dark:text-gray-100">Ny inspeksjonsrapport</h2>
          <button onClick={onClose} className="text-slate-400 dark:text-gray-500 hover:text-slate-700 dark:hover:text-gray-300 transition-colors">
            <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
            </svg>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-3">
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-medium text-slate-600 dark:text-gray-400 mb-1">Utstyr</label>
            ...
            <div>
              <label className="block text-xs font-medium text-slate-600 dark:text-gray-400 mb-1">Rapporttype</label>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-medium text-slate-600 dark:text-gray-400 mb-1">Utført av *</label>
              <input required value={performedBy} onChange={(e) => setPerformedBy(e.target.value)}
                className={inputCls}
                placeholder="Navn på tekniker" />
            </div>
            <div>
              <label className="block text-xs font-medium text-slate-600 dark:text-gray-400 mb-1">Dato</label>
          ...
          <div>
            <label className="block text-xs font-medium text-slate-600 dark:text-gray-400 mb-2">Tilstandsscore *</label>
                ...
                <button
                  ...
                  className={`flex-1 py-2 rounded-lg text-sm font-semibold transition-colors border ${
                    conditionScore === s
                      ...
                      : "bg-white dark:bg-gray-800 text-slate-500 dark:text-gray-400 border-slate-200 dark:border-gray-600 hover:border-slate-400 dark:hover:border-gray-500"
                  }`}
                >
                  {s}
                </button>
          ...
          <button
            ...
            disabled={generating}
            className="flex items-center justify-center gap-2 border border-brand-300 dark:border-brand-700 text-brand-700 dark:text-brand-400 rounded-lg py-2 text-sm font-medium hover:bg-brand-50 dark:hover:bg-brand-900/20 transition-colors disabled:opacity-60"
          >
            {generating ? (
              <>
          ...
          <div>
            <label className="block text-xs font-medium text-slate-600 dark:text-gray-400 mb-1">Observerte problemer</label>
          ...
          <div>
            <label className="block text-xs font-medium text-slate-600 dark:text-gray-400 mb-1">Utførte tiltak</label>
          ...
          <div>
            <label className="block text-xs font-medium text-slate-600 dark:text-gray-400 mb-1">Anbefalte tiltak</label>
          ...
          <div>
            <label className="block text-xs font-medium text-slate-600 dark:text-gray-400 mb-1">Neste inspeksjonsdato</label>
...
function ReportDetailModal({ report, onClose }: { report: InspectionReport; onClose: () => void }) {
  return (
    <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
      <div className="bg-white dark:bg-gray-900 rounded-2xl w-full max-w-md shadow-xl p-6 flex flex-col gap-4 max-h-[85vh] overflow-y-auto">
        <div className="flex items-center justify-between">
          <div>
            <p className="text-xs text-slate-400 dark:text-gray-500">{new Date(report.report_date).toLocaleDateString("nb-NO", { day: "2-digit", month: "long", year: "numeric" })}</p>
            <h2 className="font-semibold text-slate-900 dark:text-gray-100 mt-0.5">
              {REPORT_TYPE_LABELS[report.report_type] ?? report.report_type}
              {report.assets && <span className="text-slate-400 dark:text-gray-500 font-normal"> — {report.assets.name}</span>}
            </h2>
          </div>
          <button onClick={onClose} className="text-slate-400 dark:text-gray-500 hover:text-slate-700 dark:hover:text-gray-300 transition-colors">
            <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div className="flex items-center gap-3">
          ...
          <p className="text-sm text-slate-500 dark:text-gray-400">av {report.performed_by}</p>
        </div>

        {report.observed_issues && (
          <div>
            <p className="text-xs font-medium text-slate-500 dark:text-gray-400 mb-1">Observerte problemer</p>
            <p className="text-sm text-slate-700 dark:text-gray-300 leading-relaxed">{report.observed_issues}</p>
          </div>
        )}

        {report.actions_taken && (
          <div>
            <p className="text-xs font-medium text-slate-500 dark:text-gray-400 mb-1">Utførte tiltak</p>
            <p className="text-sm text-slate-700 dark:text-gray-300 leading-relaxed">{report.actions_taken}</p>
          </div>
        )}
    ...
    <AppShell>
      <div className="flex flex-col flex-1 overflow-auto">
        ...
        <div className="flex items-center justify-between px-6 py-4 bg-white dark:bg-gray-900 border-b border-slate-200 dark:border-gray-800 shrink-0">
          <h1 className="text-lg font-bold text-slate-900 dark:text-gray-100">Inspeksjonsrapporter</h1>
          <button
        ...
        {/* Asset health pills */}
        {assets.length > 0 && (
          <div className="px-6 py-3 bg-white dark:bg-gray-900 border-b border-slate-100 dark:border-gray-800 flex gap-2 overflow-x-auto shrink-0">
            <button
              onClick={() => setAssetFilter("")}
              className={`text-xs font-medium px-3 py-1.5 rounded-full whitespace-nowrap transition-colors ${!assetFilter ? "bg-brand-600 text-white" : "bg-slate-100 dark:bg-gray-800 text-slate-600 dark:text-gray-400 hover:bg-slate-200 dark:hover:bg-gray-700"}`}
            >
              Alle
            </button>
            {assets.map((a) => (
              <button
                ...
                className={`text-xs font-medium px-3 py-1.5 rounded-full whitespace-nowrap transition-colors ${assetFilter === a.id ? "bg-brand-600 text-white" : "bg-slate-100 dark:bg-gray-800 text-slate-600 dark:text-gray-400 hover:bg-slate-200 dark:hover:bg-gray-700"}`}
              >
                {a.name}
              </button>
            ))}
        ...
        {!assetFilter && assets.length > 0 && (
          <div className="px-6 py-4 border-b border-slate-100 dark:border-gray-800 shrink-0">
            <p className="text-xs font-medium text-slate-400 dark:text-gray-600 uppercase tracking-wide mb-3">Utstyrshelsetatus</p>
            <div className="flex gap-3 overflow-x-auto pb-1">
              {assets.slice(0, 8).map((a) => (
                <button
                  key={a.id}
                  onClick={() => setHealthAsset(a)}
                  className="flex-shrink-0 bg-white dark:bg-gray-900 border border-slate-200 dark:border-gray-800 rounded-xl px-4 py-3 text-left hover:border-brand-300 dark:hover:border-brand-700 hover:shadow-sm transition-all min-w-[140px]"
                >
                  <p className="text-xs font-semibold text-slate-700 dark:text-gray-300 truncate">{a.name}</p>
            ...
            <div className="flex flex-col gap-2">
              {filtered.map((report) => (
                <button
                  ...
                  className="bg-white dark:bg-gray-900 border border-slate-200 dark:border-gray-800 rounded-xl px-4 py-3.5 flex items-center gap-4 hover:border-slate-300 dark:hover:border-gray-700 hover:shadow-sm transition-all text-left w-full"
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      {report.assets && (
                        <span className="text-xs text-slate-400 dark:text-gray-500 truncate">— {report.assets.name}</span>
                      )}
                    </div>
                    <p className="text-xs text-slate-400 dark:text-gray-500 mt-0.5">av {report.performed_by}</p>
                  </div>
