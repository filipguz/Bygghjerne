...
export default function ThreeViewer({ project }: Props) {
  ...
  useEffect(() => {
    ...
    const ground = new THREE.Mesh(
      ...
      new THREE.MeshLambertMaterial({ color: 0x0c1520 })
    )
    ground.rotation.x = -Math.PI / 2
    ground.receiveShadow = true
    scene.add(ground)
    scene.add(new THREE.GridHelper(140, 28, 0x152030, 0x0f1a28))
    ...
    function addBox(geo: THREE.BoxGeometry, mat: THREE.MeshLambertMaterial, x: number, y: number, z: number) {
    ...
    // Building geometry from project data
  ...
  return (
    <div className="relative w-full h-full">
      ...
      <div className="absolute bottom-3 left-3 text-[10px] text-gray-500 bg-gray-900/70 backdrop-blur-sm px-2 py-1 rounded-md border border-gray-800 pointer-events-none">
        Prinsippmodell — ikke målfast
      </div>
      <div className="absolute top-3 right-3 text-[10px] text-gray-600 bg-gray-900/70 backdrop-blur-sm px-2 py-1 rounded-md border border-gray-800 pointer-events-none">
